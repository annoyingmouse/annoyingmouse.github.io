...
Array.prototype.unique = function (sort, sortingFunction) {
  ...
  for (var i = 0; i < this.length; i++) {
    if (array.inArray(this[i]) === false) array.push(this[i]);
  }
  if (sort === true) {
    if (typeof sortingFunction === "function") array.sort(sortingFunction);
    else array.sort();
  }
  return array;
};
...
function datestamp(date) {
  if (
    isNaN(
      (date =
        typeof date !== "undefined" ? new Date(date) : new Date()).getTime(),
    )
  ) {
    return null;
  }
  ...
  for (var a = tokens.length, i = 0; i < a; i++) {
    for (var b = tokens[i].length, j = 0; j < b; j++) {
      tokens[i][j] = pad2(
        date["getUTC" + tokens[i][j][0]]() + (tokens[i][j][1] || 0),
      );
    }
  }
